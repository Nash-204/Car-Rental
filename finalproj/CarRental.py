...
def edit_booking(row_index):
    ...
    # Set the selected_car to the current car of the booking being edited
    selected_car = booking_to_edit["Car"]
    ...
    for i, car in enumerate(cars):
        ...
        car_button = CTkButton(master=edit_window, text=f"{car['name']} (₱{car['price']}/day)", image=ctk_car_image,
                               compound="top",
                               command=lambda c=car["name"]: update_selected_car(c))
...
def save_edited_booking(row_index, name, date, duration, window):
    ...
    if name and date and duration and selected_car:
        ...
        update_reservation_table()
        window.destroy()
        command_prompt.configure(text="Booking updated successfully!")
    else:
...
def process_payment():
    ...
    try:
        reservation_index = int(reservation_number) - 1
        ...
        show_receipt(booking_info, total_price, payment_method)
...
def show_receipt(booking_info, total_price, payment_method):
    ...
    # Load and resize the car rental image
    car_rental_image = Image.open("Car Rental.png")
    car_rental_image = car_rental_image.resize((200, 100))
    car_rental_photo = ImageTk.PhotoImage(car_rental_image)

    # Display car rental image
    car_rental_label = tk.Label(receipt_window, image=car_rental_photo)
    car_rental_label.image = car_rental_photo
    car_rental_label.pack(pady=10)

    # Display receipt title
    title_label = tk.Label(receipt_window, text="RECEIPT", font=title_font)
    title_label.pack()

    # Display current date
    date_label = tk.Label(receipt_window, text="Date: " + datetime.datetime.now().strftime("%m-%d-%Y"), font=info_font)
    date_label.pack()

    # Create a table-like format for displaying booking information
    table_frame = tk.Frame(receipt_window)
    table_frame.pack(pady=10)

    # Define titles and corresponding values
    titles = ["Name:", "Date:", "Duration:", "Car:", "Total Price:", "Payment Method:"]
    values = [booking_info['Name'], booking_info['Date'], f"{booking_info['Duration']} days",
              booking_info['Car'], f"₱{total_price}", payment_method]

    # Display titles and values in a table-like format
    for i in range(len(titles)):
        title = tk.Label(table_frame, text=titles[i], font=info_font, anchor="w", width=15)
        title.grid(row=i, column=0, sticky="w", padx=5, pady=2)

        value = tk.Label(table_frame, text=values[i], font=info_font, anchor="e", width=25)
        value.grid(row=i, column=1, sticky="e", padx=5, pady=2)

    # Display separation line
    separator = tk.Label(receipt_window, text="----------------------------------------", font=info_font)
    separator.pack(pady=5)

    # Display thank you message
    thank_you_label = tk.Label(receipt_window, text="Thank you for choosing CarToGO!", font=info_font)
    thank_you_label.pack()

    # Close button
    close_button = tk.Button(receipt_window, text="Close", command=receipt_window.destroy, width=10)
